...
### 다음 금융 자동 동기화 수정 (v1.0.5)
- **카카오 캡처 자동 동기화가 깨져 있었음.** 같은 일을 하는 코드가 두 벌이었고 한쪽만 고쳐진 상태
  | 핸들러 | 종목 조회 | A 접두사 |
  |---|---|---|
  | `daum:addItem` (수동, DaumSync 화면) | GET 먼저 → 없으면 POST | 제거함 |
  | `daum:syncTrade` (카카오 자동) | 바로 POST | **제거 안 함** |
  - `A0193T0` 을 그대로 전송 → 다음 금융은 A 없는 코드를 받음
  - itemId 비교도 `it.symbolCode === trade.stockCode` 라 A 유무 때문에 항상 실패
  - ETF만이 아니라 **모든 종목**이 대상이었음
  - 바로 POST 하는 방식이라 중복 종목 생성 위험도 그대로 남아 있었음
- **왜 몰랐나**: 호출부가 결과를 완전히 버림
  ```js
  try { await window.api.daum.syncTrade({...}) } catch { /* 무시 */ }
  ```
  `syncTrade` 는 실패 시 예외가 아니라 `{success:false}` 를 반환하므로 catch 로 안 잡히고
  반환값도 안 봄 → 전부 실패해도 화면엔 "새로 N건 저장" 만 표시됨
- 수정
  - `daumFindOrAddItem()` 공용 헬퍼로 통합 (A 접두사 제거, GET 우선 조회,
    접두사 무관 비교, 없으면 POST 후 재조회). 두 경로가 같은 로직을 쓰도록 함
  - `DataInput` 에서 동기화 성공/실패 건수 집계 후 캡처 결과 메시지에 표시
    (실패 시 ✅ 대신 ⚠️ + 사유)
  - 메인 프로세스에 `[DAUM]` 로그 추가
  - 동기화 실패해도 매매 저장 자체는 유지 (다음 금융은 보조 기능)
- **미검증**: 실제 API 호출은 로그인 쿠키가 필요해 코드 검토와 빌드까지만 확인.
  다음 카카오 캡처 때 "다음 금융 N건 동기화" 표시와 `[DAUM]` 로그로 확인 필요
