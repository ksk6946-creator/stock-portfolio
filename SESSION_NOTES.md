...
### 잔고 대조 / 보정 기능 (v1.0.8)
- 문제: 매매내역 기준 수량과 실제 잔고가 안 맞음. 원인 3가지
  | 원인 | 사례 | 규모 |
  |---|---|---|
  | 해외 매매내역 자체가 없음 | 미국/다인 계좌 해외종목 전부 매매건수 0 | 약 1,020주 |
  | 타 증권사 이전분 | 한국카본 +113, HD현대 +113, HD현대중공업 +41, SK +14, 현대모비스 +5 | 약 286주 |
  | 종목명 변경 | 현대미포조선 / 에이치디현대미포 (매매엔 남고 잔고엔 없음) | 194주 |
- 해결: **잔고 대조/보정** 기능. 계좌 잔고 화면 → `🔍 잔고 대조`
  - 잔고(증권사 CSV = 정답) 기준으로 차이만큼 보정 거래를 생성
  - `source: 'adjustment'` 로 표시. 재실행 시 기존 보정 거래를 교체하므로 멱등
  - **잔고(holdings)는 절대 수정하지 않음**
  - 기본 선택은 부족분(diff > 0)만. 과다분(종목명 변경 추정)은 사용자가 직접 판단
  - `reconcile:get` / `reconcile:apply` / `reconcile:clear` IPC
- **중요: 보정 거래는 반드시 매매내역 "마지막" 날짜에 넣어야 함**
  - 처음엔 최초 매매 하루 전에 넣었는데 21종목 중 4종목이 목표 수량에 도달하지 못했고
    멱등성도 깨졌음 (재실행 시 또 차이 발생)
  - 이유: 매수가 누락돼 이미 수량이 0으로 clamp 된 구간이 있어서,
    앞에 넣은 보정분을 이후 매도 거래가 소진해버림
  - 마지막에 넣으면 `최종수량 = 기존 + 차이` 가 되어 수량·평단 모두 정확히 일치
  - 검증: 21종목 전부 수량/평단 일치, 재실행 시 추가 차이 0건
- 보정 매수 단가는 매매내역 평단이 잔고 평단과 같아지도록 역산
  `price = (잔고수량 × 잔고평단 − 기존취득원가) / 차이`
  - 역산값이 음수면 잔고 평단 사용
- 주의: `deleteTrade`/`updateTrade` 는 `recalcHolding` 을 호출해 잔고를 덮어씀.
  보정 로직은 이를 피하려고 `data.trades` 를 직접 필터링함

### 다음 금융 수량 불일치 수정 (v1.0.9)
- 상황: 앱 잔고는 실제 계좌와 맞는데 다음 금융 보유수량이 안 맞음.
  다음 금융은 매매내역만 받아서 만들어지므로, 이전받은 주식만큼 부족해짐
- 조치 1: 메인 계좌 5종목에 보정 거래 생성 (잔고 대조 기능)
  현대모비스 +5, 한국카본 +113, SK +14, HD현대 +113, HD현대중공업 +41
  → 24종목 전부 수량 일치, 22종목 평단까지 일치
- 조치 2: **동기화 중복 판정 버그 2건 수정**
  - `existingKeys` 가 Set 이라 같은 날 같은 가격 분할매수가 2건이면 1건만 있는 것으로 보고
    **둘 다 건너뜀** → `Map<key, count>` 멀티셋으로 변경해 개수까지 비교
  - `syncedTradeIds`(설정에 영구 저장)로 스킵하던 로직 제거.
    그룹을 삭제/재생성하면 다음 금융은 비었는데 앱은 "이미 올렸다"고 기억해서 전부 건너뜀
    (현대모비스 42건 중 41건이 이 상태였음)
    → 이제 다음 금융의 **실제 내역만** 기준으로 판정하므로 자동으로 복구됨
- 조치 3: 동기화 후 검증 추가
  - `daum:getItems` IPC 신규 (그룹의 종목 + holdingVolume 조회)
  - 동기화 완료 후 다음 금융 보유수량 vs 앱 잔고를 비교해 불일치를 로그로 출력
  - `🔎 수량 검증` 버튼으로 단독 실행도 가능

### 흡수합병 종목명 통합 — 시도했으나 되돌림
- 현대미포조선 + 에이치디현대미포 → HD현대중공업 (실제로 흡수합병됨)
- 이름을 합쳐봤더니 **오히려 나빠져서 백업으로 되돌렸음**
  - 합병은 주식 교환비율이 있어서 미포 106주가 HD현대중공업 106주가 되지 않음
  - 이름만 합치면 수량이 부풀려짐 (46주여야 하는데 108주로 계산됨)
  - 보정 매도로 46주까지 줄일 수는 있지만, **이동평균법에서 매도는 평단을 바꾸지 못함**
    → 평단이 191,648 로 남음 (실제 392,539)
- 결론: 옛 이름을 그대로 두는 편이 낫다
  - 잔고에 없으므로 다음 금융 동기화 기본 선택에서 자동 제외됨 (실害 없음)
  - 매매내역상 유령 수량으로만 남음 (현대미포조선 17주, 에이치디현대미포 89주)
  - 제대로 하려면 합병 이벤트(교환비율, 기준일)를 모델링해야 함. 현재는 미지원

### 보정 수량 계산 오류 수정 (v1.0.10) — 중요
- 증상: v1.0.9 로 동기화했더니 24종목 중 20종목만 일치.
  보정을 넣은 5종목 중 4종목이 실패 (현대모비스 0, SK 0, 한국카본 3, HD현대중공업 0)
- 실제 다음 금융 API 를 직접 조회해서 원인 규명
  - 한국카본: 37건 전부 저장돼 있고 **매수합 167 - 매도합 64 = 103** 인데
    `holdingVolume` 은 **3**
  - 즉 **다음 금융도 보유수량이 0 아래로 내려가면 잘라낸다(clamp)**
  - 이전받은 주식은 매수 기록이 없어서 중간에 매도가 보유량을 초과하고, 그 시점에 수량이 소실됨
- **내가 틀린 두 가지**
  1. 보정 수량을 **clamp 된 값** 기준으로 계산했음 (한국카본 113) →
     실제 필요한 건 **단순합산(매수합-매도합)** 기준 (131)
  2. 보정 거래를 **맨 뒤**에 넣었음 → 중간 음수 구간이 그대로 남아 수량이 소실됨.
     **맨 앞**(최초 매매 하루 전)에 넣어야 처음부터 보유한 것이 되어 음수 구간이 사라짐
- 수정 후 (메인 계좌 5종목, 전부 수량·평단 일치)
  | 종목 | 합산(보정제외) | 보정 | 단가 | 결과 |
  |---|---|---|---|---|
  | 현대모비스 | -12 | +18 | 283,999 | 6주 / 329,747 |
  | 한국카본 | -10 | +131 | 12,318 | 121주 / 17,672 |
  | SK | -17 | +32 | 173,356 | 15주 / 206,224 |
  | HD현대 | 183 | +113 | 76,156 | 296주 / 130,252 |
  | HD현대중공업 | 4 | +42 | 401,540 | 46주 / 392,538 |
- 보정 단가는 **선형성을 이용해 역산**
  - 이동평균법에서 매도는 원가를 비례 차감하므로 최종 취득원가는 보정 단가에 대해 선형
  - `simulateCost(단가=0)`, `simulateCost(단가=1)` 두 점으로 기울기를 구해
    목표 취득원가(잔고수량×잔고평단)를 만드는 단가를 역산
- `computeRawNet()` 추가: 단순합산 + `minRunning`(보정 없이 도달하는 최저 수량).
  `minRunning + 보정수량 >= 0` 이면 음수 구간이 사라진다는 것을 검증에 사용
- 다음 금융에 이미 올라간 **잘못된 보정 거래 5건은 API 로 직접 삭제**했음
  (2026-08-03 자, 이전 단가/수량으로 식별). 삭제 후 HD현대 296→183 으로 감소 확인
- 남은 평단 미세 차이 2건: HD한국조선해양 9원, 피에스케이(무상증자 추정)

**교훈**: 다음 금융의 계산 방식을 추측하지 말고 API 를 직접 조회해서 확인할 것.
`holdingVolume`, `averagePurchasePrice` 필드가 실제 값을 담고 있다.
